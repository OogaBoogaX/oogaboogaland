...
const mirrorCave = () => withPage("mirror cave", hubPage(src), async (b) => {
  ...
  const exteriorFacingNames = ["transitionHeader", "doorwayLintel", "transitionReturns", "doorwayJambs"];
  ...
  await rendered(16);
  const matrixAfter = await b.evaluate(`(() => { const B = window.__ooga; return { y: B.matrixCave.firstGlyphY, updates: B.matrixCave.updates, version: B.matrixCave.glyphVersion, hash: B.matrixCave.mutationHash, active: B.matrixCave.activeGlyphCount, gaps: B.matrixCave.movingGapCount, drawn: B.matrixCave.drawnGlyphCount, drawEnabled: B.matrixCave.drawEnabled, allocations: B.matrixCave.allocationCount, rebuilds: B.matrixCave.rebuildCount, passes: B.mirror.reflectionPassCount, surfaceDrawn: B.mirror.surfaceDrawn, reason: B.mirror.skipReason, resources: B.mirror.resources }; })()`);
  ...
  const namedSurfaceCounts = [surfaces.mainFloor, surfaces.vestibuleFloor, surfaces.mainCeiling, surfaces.vestibuleCeiling, surfaces.mainLeftWall, surfaces.mainRightWall, surfaces.vestibuleLeftWall, surfaces.vestibuleRightWall, surfaces.backWall, surfaces.transitionHeader, surfaces.doorwayLintel, surfaces.transitionReturns, surfaces.doorwayJambs, surfaces.rimUnderside];
  record("mirror interior: tightly packed code covers every physical black chamber surface in its required direction", matrixBefore.matrix.surfaceGlyphCount > 9000 && matrixBefore.matrix.surfacePitch === 0.12 && matrixBefore.matrix.surfaceGap === 0.13 && namedSurfaceCounts.every((count) => count > 0) && surfaces.leftWall === surfaces.rightWall && [streams.floor, streams.ceiling, streams.leftWall, streams.rightWall, streams.backWall, streams.doorway].every((count) => count > 0) && layout.floor.plane === "horizontal" && layout.floor.flow === "entrance-to-back" && layout.ceiling.flow === "entrance-to-back" && layout.leftWall.plane === "vertical" && layout.leftWall.flow === "down" && layout.rightWall.flow === "down" && layout.backWall.flow === "down", JSON.stringify({ surfaces, streams, layout }));
  record("mirror interior: the doorway stays open and all surface code remains behind the mirror plane", layout.doorway.minY > layout.doorway.portalMaxY && layout.doorway.z < matrixBefore.matrix.portal.opening.planeZ && layout.floor.maxZ < matrixBefore.matrix.portal.opening.planeZ && matrixBefore.matrix.surfaceCounts.doorway > 0, JSON.stringify({ doorway: layout.doorway, floor: layout.floor, opening: matrixBefore.matrix.portal.opening }));
  const edgeMargins = Object.values(matrixBefore.matrix.surfaceEdgeMargins);
  record("mirror interior: every registered surface grid reaches its shared seams", edgeMargins.length === 14 && edgeMargins.every((margin) => margin < matrixBefore.matrix.surfacePitch * 0.5), JSON.stringify({ edgeMargins, halfPitch: matrixBefore.matrix.surfacePitch * 0.5 }));
  record("mirror interior: deterministic stream brightness, train lengths and moving blank gaps vary within bounds", matrixBefore.matrix.minBrightness >= 0.58 && matrixBefore.matrix.maxBrightness <= 0.94 && matrixBefore.matrix.maxBrightness - matrixBefore.matrix.minBrightness > 0.3 && matrixBefore.matrix.minTrainLength >= 1 && matrixBefore.matrix.maxTrainLength === 14 && matrixBefore.matrix.minGapLength >= 1 && matrixBefore.matrix.maxGapLength >= 6 && matrixBefore.matrix.movingGapCount > 1000 && matrixBefore.matrix.maxLocalZ < matrixBefore.matrix.portal.opening.planeZ && matrixBefore.matrix.portalClearance > 0.01, JSON.stringify({ brightness: [matrixBefore.matrix.minBrightness, matrixBefore.matrix.maxBrightness], trains: [matrixBefore.matrix.minTrainLength, matrixBefore.matrix.maxTrainLength], gaps: [matrixBefore.matrix.minGapLength, matrixBefore.matrix.maxGapLength], movingGaps: matrixBefore.matrix.movingGapCount, maxLocalZ: matrixBefore.matrix.maxLocalZ, clearance: matrixBefore.matrix.portalClearance }));
  ...
  const matrixTiers = await b.evaluate(`new Promise((resolve) => { const B = window.__ooga, samples = [], names = ["high", "medium", "low", "high"], surfaceNames = ["mainFloor", "vestibuleFloor", "mainCeiling", "vestibuleCeiling", "mainLeftWall", "mainRightWall", "vestibuleLeftWall", "vestibuleRightWall", "backWall", "transitionHeader", "doorwayLintel", "transitionReturns", "doorwayJambs", "rimUnderside"], sample = () => { const a = B.matrixCave.activeSurfaceCounts; samples.push({ quality: B.matrixCave.quality, density: B.matrixCave.qualityDensity, surfaceActive: a.floor + a.ceiling + a.leftWall + a.rightWall + a.backWall + a.doorway, named: surfaceNames.map((name) => a[name]), allocations: B.matrixCave.allocationCount, rebuilds: B.matrixCave.rebuildCount, buffers: B.matrixCave.bufferCount, bytes: B.matrixCave.bufferBytes }); }, next = (i) => { if (i === names.length) return resolve(samples); B.renderer.setQuality(names[i]); const start = B.renderedFrames, tick = () => { if (B.renderedFrames >= start + 3) { sample(); next(i + 1); } else requestAnimationFrame(tick); }; requestAnimationFrame(tick); }; next(0); })`);
  record("mirror interior: quality tiers retain every named surface while reducing one deterministic registry", matrixTiers.map((s) => s.quality).join("|") === "high|medium|low|high" && matrixTiers[0].density === 1 && matrixTiers[1].density === 0.625 && matrixTiers[2].density === 0.375 && matrixTiers[0].surfaceActive > matrixTiers[1].surfaceActive && matrixTiers[1].surfaceActive > matrixTiers[2].surfaceActive && matrixTiers[0].surfaceActive === matrixTiers[3].surfaceActive && matrixTiers.every((s) => s.named.every((count) => count > 0) && s.allocations === 8 && s.rebuilds === 1 && s.buffers === 8 && s.bytes === matrixTiers[0].bytes), JSON.stringify(matrixTiers));
...
const mirrorCanvas = () => withPage("mirror canvas fallback", hubPage(src, "canvas2d=1&bananas=1000000&hour=22"), async (b) => {
  ...
  const canvasNamed = [matrix.surfaces.mainFloor, matrix.surfaces.vestibuleFloor, matrix.surfaces.mainCeiling, matrix.surfaces.vestibuleCeiling, matrix.surfaces.mainLeftWall, matrix.surfaces.mainRightWall, matrix.surfaces.vestibuleLeftWall, matrix.surfaces.vestibuleRightWall, matrix.surfaces.backWall, matrix.surfaces.transitionHeader, matrix.surfaces.doorwayLintel, matrix.surfaces.transitionReturns, matrix.surfaces.doorwayJambs, matrix.surfaces.rimUnderside];
  record("mirror interior: Canvas fallback draws a bounded depth-sorted approximation on every surface", matrix.scene === "hub" && matrix.kind === "canvas2d" && matrix.active && matrix.portal && !matrix.surfaceDrawn && matrix.faux && matrix.resources === 0 && matrix.streams === 32 && matrix.hanging === 18 && matrix.entrance === 12 && matrix.walls === 14 && matrix.rainGlyphs === 288 && matrix.surfaceGlyphs > 9000 && matrix.activeGlyphs > 1000 && canvasNamed.every((count) => count > 0) && matrix.tips > 80 && matrix.capacity >= matrix.rainGlyphs + matrix.surfaceGlyphs && matrix.quality === "canvas2d" && matrix.density === 0.125 && matrix.buffers === 8 && matrix.bytes < 800000 && matrix.allocations === 8 && matrix.rebuilds === 1 && matrix.visible && matrix.inside && matrix.updates > 0, JSON.stringify(matrix));
  const canvasMotion = await b.evaluate(`new Promise((resolve) => { const B = window.__ooga, before = B.matrixCave.sampleMotion("mainFloor"), start = B.renderedFrames, tick = () => { if (B.renderedFrames >= start + 10) resolve({ before, after: B.matrixCave.sampleMotion("mainFloor") }); else requestAnimationFrame(tick); }; requestAnimationFrame(tick); })`);
