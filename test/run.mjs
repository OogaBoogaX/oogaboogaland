...
const shellCoverage = `(level) => { const B = window.__ooga, M = window.BL.models, profile = M.BANANA_PILE_PROFILE; B.setPileLevel(level); const data = B.shell.instanceData, n = B.shell.instanceCount, core = B.core; const surfaceY = (r) => { for (let i = 0; i < profile.length - 1; i++) { const o = profile[i], q = profile[i + 1]; if (r < q[0]) continue; return o[1] + (q[1] - o[1]) * (o[0] - r) / (o[0] - q[0]); } return profile[profile.length - 1][1]; }; let worst = 0, sum = 0; const samples = 600; for (let s = 0; s < samples; s++) { const u = (s + 0.5) / samples, angle = s * 2.399963; const r = Math.sqrt(u) * 0.97, wr = r * M.bananaPileRadiusScale(angle, r) * core.scale.x, x = Math.cos(angle) * wr, z = Math.sin(angle) * wr, y = core.position.y + (surfaceY(r) + M.bananaPileHeightOffset(angle, r)) * core.scale.y; let nearest = Infinity; for (let i = 0; i < n; i++) { const o = i * 20, d = Math.hypot(data[o + 12] - x, data[o + 13] - y, data[o + 14] - z); if (d < nearest) nearest = d; } worst = Math.max(worst, nearest); sum += nearest; } return { level, tiles: n, worstGap: +worst.toFixed(3), meanGap: +(sum / samples).toFixed(3), brightest: Math.max(...core.geometry.faces.map((f) => Math.max(...f.color))) }; }`;
const covered = (c) => c.worstGap <= 0.14 && c.meanGap <= 0.08 && c.brightest < 90 && c.tiles > 0;
const results = [];
...
const core = (label, base) => withPage(label, page(base), async (b) => {
  ...
  record(`${label}: overlapping ammo-size shell bananas cover the mound with no banana on top of another`, shell.looseVisible === 0 && covered(shell.coverage) && shell.coverage.tiles >= 120 && shell.coverage.tiles <= 260 && shell.maxScaleError < 1e-6 && shell.minCenter > 0.02 && shell.maxTurn > 0.3 && shell.depth > 0.06 && shell.faces >= 40 && shell.apex < 0.85 && shell.apex - shell.shoulder < 0.03, JSON.stringify(shell));
  const redrawn = await b.evaluate(`(() => { const B = window.__ooga, beforeVersion = B.shell.instanceVersion, before = B.shell.instanceCount, edge = B.core.scale.x; B.setPileLevel(1001); const oneMore = { version: B.shell.instanceVersion, edge: B.core.scale.x }; B.setPileLevel(1100); const grown = { version: B.shell.instanceVersion, tiles: B.shell.instanceCount, edge: B.core.scale.x }; B.setPileLevel(1000); return { beforeVersion, before, edge, oneMore, grown, back: { version: B.shell.instanceVersion, tiles: B.shell.instanceCount, edge: B.core.scale.x } }; })()`);
  record(`${label}: the mound grows with every banana and the shell relays only when it needs more bananas`, redrawn.oneMore.edge > redrawn.edge && redrawn.oneMore.version === redrawn.beforeVersion && redrawn.grown.version === redrawn.beforeVersion + 1 && redrawn.grown.tiles > redrawn.before && redrawn.grown.edge > redrawn.oneMore.edge && redrawn.back.version === redrawn.grown.version + 1 && redrawn.back.tiles === redrawn.before && redrawn.back.edge === redrawn.edge, JSON.stringify(redrawn));
...
const mirrorCave = () => withPage("mirror cave", hubPage(src), async (b) => {
  ...
  const built = await b.evaluate(`(() => { const B = window.__ooga, C = B.mirrorCave, slot = window.BL.caves.slots.find((s) => s.id === "c1"), g = C.node.geometry, rimZ = C.rim.geometry.verts.filter((_, i) => i % 3 === 2).map((z) => z + C.rim.position.z), rimMinZ = Math.min(...rimZ), rimMaxZ = Math.max(...rimZ), min = [Infinity, Infinity, Infinity], max = [-Infinity, -Infinity, -Infinity]; for (let i = 0; i < g.verts.length; i += 3) for (let a = 0; a < 3; a++) { min[a] = Math.min(min[a], g.verts[i + a]); max[a] = Math.max(max[a], g.verts[i + a]); } const entropy = window.BL.hubModels.caveSign("EntropyLab"), ooga = window.BL.hubModels.caveSign("Ooga Booga Land"), label = B.labels.find((l) => l.text === "Ooga Booga Land"); return { status: slot.status, name: slot.name, scene: slot.scene, children: C.group.children.length, mirrorMarked: C.node.mirror === true, attached: C.node.parent === C.group && C.rim.parent === C.group && C.sign.parent === C.group, insideRim: C.node.position.z > rimMinZ && C.node.position.z < rimMaxZ, rimFrontGap: rimMaxZ - C.node.position.z, worldBottom: C.node.position.y + min[1], bounds: { min, max }, sign: { label: label && label.text, cached: ooga === window.BL.hubModels.caveSign("Ooga Booga Land") && entropy === window.BL.hubModels.caveSign("EntropyLab"), wider: ooga.signWidth > entropy.signWidth, faces: ooga.faces.length } }; })()`);
  record("mirror cave: c1 has its sign and a full-opening mirror inside an otherwise empty rim", built.status === "mirror" && built.name === "Ooga Booga Land" && built.scene === null && built.children === 3 && built.mirrorMarked && built.attached && built.insideRim && built.rimFrontGap >= 0.49 && built.worldBottom < 0 && built.bounds.min[0] === -2.5 && built.bounds.max[0] === 2.5 && built.bounds.min[1] === -1.75 && built.bounds.max[1] === 1.5 && built.bounds.min[2] === 0 && built.bounds.max[2] === 0 && built.sign.label === built.name && built.sign.cached && built.sign.wider && built.sign.faces > 100, JSON.stringify(built));
  ...
  record("mirror cave: high target is bounded and uses one RGBA8 plus one depth16 surface", Math.max(reflected.width, reflected.height) <= 512 && aspect(reflected) < 0.01 && reflected.resources === 4 && reflected.allocations === 1 && reflected.bytes < 512 * 512 * 6, JSON.stringify(reflected));
  const quality = async (name, cap) => {
    const sample = await b.evaluate(`new Promise((resolve) => { const B = window.__ooga; B.renderer.setQuality(${JSON.stringify(name)}); const startFrame = B.renderedFrames, startPass = B.mirror.reflectionPassCount, t0 = performance.now(); const tick = () => { if (B.renderedFrames >= startFrame + 8 || performance.now() - t0 > 6000) resolve({ name: B.renderer.quality, width: B.mirror.width, height: B.mirror.height, viewport: [B.renderer.size.width, B.renderer.size.height], passes: B.mirror.reflectionPassCount - startPass, resources: B.mirror.resources }); else requestAnimationFrame(tick); }; requestAnimationFrame(tick); })`);
    sample.cap = cap;
    return sample;
  };
  const tiers = [await quality("high", 512), await quality("medium", 384), await quality("low", 256)];
  record("mirror cave: quality tiers preserve aspect and medium/low update every second frame", tiers.every((s) => s.name === (s.cap === 512 ? "high" : s.cap === 384 ? "medium" : "low") && Math.max(s.width, s.height) <= s.cap && aspect(s) < 0.01 && s.resources === 4) && tiers[0].passes >= 7 && tiers[1].passes >= 3 && tiers[1].passes <= 5 && tiers[2].passes >= 3 && tiers[2].passes <= 5, JSON.stringify(tiers));
  await b.evaluate(`window.__ooga.renderer.setQuality("high")`);
  await rendered(4);
  const skips = await b.evaluate(`new Promise((resolve) => { const B = window.__ooga, node = B.mirrorCave.node, wait = (count, done) => { const start = B.renderedFrames, tick = () => B.renderedFrames >= start + count ? done() : requestAnimationFrame(tick); requestAnimationFrame(tick); }, start = { pass: B.mirror.reflectionPassCount, skip: B.mirror.skippedPassCount }; node.rotation.y = Math.PI; wait(3, () => { const back = { pass: B.mirror.reflectionPassCount, skip: B.mirror.skippedPassCount, reason: B.mirror.skipReason }; node.rotation.y = 0; node.position.x = 20; wait(3, () => { const offscreen = { pass: B.mirror.reflectionPassCount, skip: B.mirror.skippedPassCount, reason: B.mirror.skipReason }; node.position.x = 0; wait(3, () => resolve({ start, back, offscreen, restored: { pass: B.mirror.reflectionPassCount, reason: B.mirror.skipReason } })); }); }); })`);
  ...
  record("mirror cave: resize rebuilds only the bounded target and keeps resource count flat", beforeResize.resources === 4 && resized.resources === 4 && resetSize.resources === 4 && resized.allocations > beforeResize.allocations && Math.max(resized.width, resized.height) <= 512 && aspect(resized) < 0.01, JSON.stringify({ beforeResize, resized, resetSize }));
  ...
  const hubResources = await b.evaluate(`(() => { const B = window.__ooga; return { scene: B.scene, active: B.mirror.active, resources: B.mirror.resources, records: B.renderer.stats.records }; })()`);
  ...
  record("mirror cave: scene cycling and context restoration release and recreate a fixed resource set", cycled.lab.scene === "lab" && !cycled.lab.active && cycled.lab.resources === 0 && cycled.hub.scene === "hub" && cycled.hub.active && cycled.hub.resources === 4 && restored.before.resources === 4 && restored.after.resources === 4 && restored.after.records === restored.before.records && Math.max(restored.after.width, restored.after.height) <= 512, JSON.stringify({ cycled, restored }));
});

const mirrorCanvas = () => withPage("mirror canvas fallback", hubPage(src, "canvas2d=1&bananas=1000000"), async (b) => {
  const r = await b.evaluate(`(() => { const B = window.__ooga, C = B.mirrorCave, candidates = B.props.filter((o) => o.scenery); return { kind: B.renderer.kind, active: B.mirror.active, faux: B.mirror.faux, resources: B.mirror.resources, passes: B.mirror.reflectionPassCount, skipped: B.mirror.skippedPassCount, children: C.group.children.length, mirrorMarked: C.node.mirror === true, path: { active: B.path.active, inner: B.path.ringInnerRadius, outer: B.path.ringOuterRadius, count: B.path.visibleInstanceCount, capacity: B.path.bufferCapacity, masterMaskBuildCount: B.path.masterMaskBuildCount, masterMaskHash: B.path.masterMaskHash }, scenery: { ...B.scenery, signature: candidates.map((o) => [o.prop, o.x, o.z, o.node.rotation.y].join(":" )).join("|") } }; })()`);
  record("mirror canvas fallback: depth-sorted faux mirror draws without reflection resources", r.kind === "canvas2d" && r.active && r.faux && r.resources === 0 && r.passes === 0 && r.skipped > 0 && r.children === 3 && r.mirrorMarked, JSON.stringify({ kind: r.kind, active: r.active, faux: r.faux, resources: r.resources, passes: r.passes, skipped: r.skipped, children: r.children, mirrorMarked: r.mirrorMarked }));
  record("dynamic path: Canvas fallback renders the same immutable million-banana network", r.path.active && r.path.inner === 7.5 && r.path.outer === 9 && r.path.count > 0 && r.path.count <= r.path.capacity && r.path.masterMaskBuildCount === 1 && r.path.masterMaskHash === pathMasterHash, JSON.stringify(r.path));
  record("dynamic scenery: Canvas fallback starts large with the same deterministic registry", r.scenery.candidateCount === 226 && r.scenery.visibleCount > 0 && r.scenery.signature === scenerySignature, JSON.stringify({ candidateCount: r.scenery.candidateCount, visibleCount: r.scenery.visibleCount, radiusCulledCount: r.scenery.radiusCulledCount, pathCulledCount: r.scenery.pathCulledCount }));
});
...
const dynamicPaths = () => withPage("dynamic paths", hubPage(src, "bananas=1000&b=100"), async (b) => {
  ...
  record("dynamic path: quantized inner edge stays at least 0.25 beyond the platform", samples.every((s) => s.inner + 1e-8 >= s.platform + 0.25 && Math.abs(s.inner / 0.25 - Math.round(s.inner / 0.25)) < 1e-8 && s.center === s.inner + 0.75 && s.outer === s.center + 0.75), JSON.stringify(samples.map(({ level, platform, requested, inner, quantized }) => ({ level, platform, requested, inner, quantized }))));
  ...
  record("dynamic scenery: visible footprints clear the ring and path at 1K, 10K and 1M", [scenery.small, scenery.medium, scenery.large].every((s) => s.radiusViolations === 0 && s.pathViolations === 0 && s.debug.visibleCount + s.debug.radiusCulledCount + s.debug.pathCulledCount + s.debug.fixedCulledCount === scenery.candidateCount), JSON.stringify({ small: scenery.small, medium: scenery.medium, large: scenery.large }));
  ...
  record("dynamic path and scenery: repeated growth and shrink keep targets and GPU resources bounded", cycles.every((row, i) => row.records === cycles[0].records && row.active === cycles[0].active && row.pathCount <= samples[0].capacity && stableCycle(row, i)) && cycles.at(-1).pathReflows >= cycles[0].pathReflows + cycles.length - 1 && cycles.at(-1).sceneryReflows >= cycles[0].sceneryReflows + cycles.length - 1, JSON.stringify(cycles));
...
const hubPile = () => withPage("hub pile", hubPage(src), async (b) => {
  ...
  record("hub: mirror and million-banana pile hold at least 50 FPS", perf.fps >= 50 && perf.shown >= 999999 && mirror.active && mirror.passes > 0 && mirror.resources === 4, `${perf.fps} fps at ${perf.shown} bananas · ${JSON.stringify(mirror)}`);
});
...
const hubProps = () => withPage("hub props", hubPage(src, "loot=1"), async (b) => {
  ...
  const near = await b.evaluate(`(() => { const B = window.__ooga; const cave = [...B.cavemen.values()].find((c) => c.state === "working" && !c.walk && !c.build); const o = B.props.find((o) => o.prop === "barrel" && o.active); B.crew.control(cave); cave.root.position.x = o.x + 0.3; cave.root.position.z = o.z; return { name: cave.traits.name, player: B.crew.player === cave }; })()`);
  await b.sleep(100);
  await b.key(" ");
  await b.sleep(200);
