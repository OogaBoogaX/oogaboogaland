...
const mirrorCave = () => withPage("mirror cave", hubPage(src), async (b) => {
  ...
  const built = await b.evaluate(`(() => { const B = window.__ooga, C = B.mirrorCave, slot = window.BL.caves.slots.find((s) => s.id === "c1"), g = C.node.geometry, room = C.room.geometry, rimZ = C.rim.geometry.verts.filter((_, i) => i % 3 === 2).map((z) => z + C.rim.position.z), rimMinZ = Math.min(...rimZ), rimMaxZ = Math.max(...rimZ), min = [Infinity, Infinity, Infinity], max = [-Infinity, -Infinity, -Infinity]; for (let i = 0; i < g.verts.length; i += 3) for (let a = 0; a < 3; a++) { min[a] = Math.min(min[a], g.verts[i + a]); max[a] = Math.max(max[a], g.verts[i + a]); } const roomFront = Math.max(...room.verts.filter((_, i) => i % 3 === 2)), rimCladdingFaces = room.faces.filter((face) => face.i.every((i) => room.verts[i * 3 + 2] >= 0.219 && room.verts[i * 3 + 2] <= 0.251) && Math.abs(face.i.reduce((sum, i) => sum + room.verts[i * 3], 0) / face.i.length) > 2.5).length, upperPanelFaces = room.faces.filter((face) => face.i.every((i) => room.verts[i * 3 + 1] >= 2.999 && room.verts[i * 3 + 2] >= 0.44)).length, outwardPortalFaces = room.faces.filter((face) => { if (!face.i.every((i) => Math.abs(room.verts[i * 3 + 2] - 0.48) < 1e-8)) return false; const a = face.i[0] * 3, b = face.i[1] * 3, c = face.i[2] * 3; return (room.verts[b] - room.verts[a]) * (room.verts[c + 1] - room.verts[a + 1]) - (room.verts[b + 1] - room.verts[a + 1]) * (room.verts[c] - room.verts[a]) > 0; }).length, exteriorRoomVertices = room.verts.filter((z, i) => i % 3 === 2 && z >= C.node.position.z).length, entropy = window.BL.hubModels.caveSign("EntropyLab"), ooga = window.BL.hubModels.caveSign("Ooga Booga Land"), label = B.labels.find((l) => l.text === "Ooga Booga Land"); return { status: slot.status, name: slot.name, scene: slot.scene, children: C.group.children.length, mirrorMarked: C.node.mirror === true, walkThrough: C.node.mirrorWalkThrough === true, roomAttached: C.room.parent === C.group, roomVisible: C.room.visible, roomScale: C.room.scale.x, rainVisible: B.matrixCave.visible, roomFront, declaredFront: room.frontZ, claddingFrontZ: room.claddingFrontZ, headerMinY: room.headerMinY, upperPanelFaces, outwardPortalFaces, mirrorPlaneZ: C.node.position.z, exteriorRoomVertices, exteriorGap: C.node.position.z - roomFront, rimCladdingFaces, transitionCladdingZ: room.transitionCladdingZ, guideFaces: room.faces.filter((face) => face.emissive > 0.5).length, blackFaces: room.faces.every((face) => face.color[0] === 0 && face.color[1] === 0 && face.color[2] === 0 && !face.emissive), attached: C.node.parent === C.group && C.rim.parent === C.group && C.sign.parent === C.group, insideRim: C.node.position.z > rimMinZ && C.node.position.z < rimMaxZ, rimFrontGap: rimMaxZ - C.node.position.z, worldBottom: C.node.position.y + min[1], bounds: { min, max }, sign: { label: label && label.text, cached: ooga === window.BL.hubModels.caveSign("Ooga Booga Land") && entropy === window.BL.hubModels.caveSign("EntropyLab"), wider: ooga.signWidth > entropy.signWidth, faces: ooga.faces.length } }; })()`);
  record("mirror cave: c1 has its sign, walk-through mirror and in-world chamber", built.status === "mirror" && built.name === "Ooga Booga Land" && built.scene === null && built.children === 5 && built.mirrorMarked && built.walkThrough && built.roomAttached && built.attached && built.insideRim && built.rimFrontGap >= 0.49 && built.worldBottom < 0 && built.bounds.min[0] === -2.5 && built.bounds.max[0] === 2.5 && built.bounds.min[1] === -1.75 && built.bounds.max[1] === 1.5 && built.bounds.min[2] === 0 && built.bounds.max[2] === 0 && built.sign.label === built.name && built.sign.cached && built.sign.wider && built.sign.faces > 100, JSON.stringify(built));
  record("mirror cave: black upper panel and interior stop completely behind the mirror plane", (!built.roomVisible || built.roomScale === 0) && !built.rainVisible && Math.abs(built.roomFront - 0.48) < 1e-8 && built.declaredFront === 0.48 && built.claddingFrontZ === 0.48 && built.headerMinY === 3 && built.upperPanelFaces > 0 && built.outwardPortalFaces === 0 && built.mirrorPlaneZ === 0.5 && built.exteriorRoomVertices === 0 && built.exteriorGap >= 0.019 && built.rimCladdingFaces === 0 && built.transitionCladdingZ === -2.485 && built.guideFaces === 0 && built.blackFaces, JSON.stringify({ roomVisible: built.roomVisible, roomScale: built.roomScale, rainVisible: built.rainVisible, roomFront: built.roomFront, declaredFront: built.declaredFront, claddingFrontZ: built.claddingFrontZ, headerMinY: built.headerMinY, upperPanelFaces: built.upperPanelFaces, outwardPortalFaces: built.outwardPortalFaces, mirrorPlaneZ: built.mirrorPlaneZ, exteriorRoomVertices: built.exteriorRoomVertices, exteriorGap: built.exteriorGap, rimCladdingFaces: built.rimCladdingFaces, transitionCladdingZ: built.transitionCladdingZ, guideFaces: built.guideFaces, blackFaces: built.blackFaces }));
  ...
  record("mirror cave: scene cycling and context restoration release and recreate a fixed resource set", cycled.lab.scene === "lab" && !cycled.lab.active && cycled.lab.resources === 0 && cycled.hub.scene === "hub" && cycled.hub.active && cycled.hub.resources === 6 && restored.before.resources === 6 && restored.after.resources === 6 && restored.after.records === restored.before.records && Math.max(restored.after.width, restored.after.height) <= 512, JSON.stringify({ cycled, restored }));
});
...
// ---------- the clock ----------
const daylightNight = () => withPage("daylight night", hubPage(src, "hour=22"), async (b) => {
  const keys = await b.evaluate(`(() => { const D = window.BL.daylight, out = { clear: [0, 0, 0], horizon: [0, 0, 0], zenith: [0, 0, 0], sky: [0, 0, 0], ground: [0, 0, 0], sun: [0, 0, 0], light: { x: 0, y: 0, z: 0 }, moon: { x: 0, y: 0, z: 0 } }; D.sample(12, out); const noon = { clear: [...out.clear], sun: [...out.sun], stars: out.stars, torch: out.torch, day: out.day, bloom: out.bloomStrength }; const phases = [0, 5, 6.9, 7, 11, 15.9, 16, 19, 22.9, 23, 23.9].map(D.phaseAt); let darkest = 1, lowest = 1; for (let h = 0; h < 24; h += 0.05) { D.sample(h, out); darkest = Math.min(darkest, out.sky[0], out.sky[1], out.sky[2], out.ground[0], out.ground[1], out.ground[2]); lowest = Math.min(lowest, out.light.y); } return { noon, phases, darkest, lowest }; })()`);
  const noonOk = Math.abs(keys.noon.clear[0] - 0.36) < 1e-6 && Math.abs(keys.noon.clear[2] - 0.82) < 1e-6 && Math.abs(keys.noon.sun[0] - 0.48) < 1e-6 && keys.noon.stars === 0 && keys.noon.torch === 0 && keys.noon.day === 1 && keys.noon.bloom === 0.5;
  ...
  record("daylight: night ambient stays readable and the light never drops below 0.25 elevation", keys.darkest >= 0.1 && keys.lowest >= 0.25 - 1e-6, JSON.stringify({ darkest: keys.darkest, lowest: keys.lowest }));
  const night = await b.evaluate(`(() => { const B = window.__ooga, o = B.renderOpts; return { hour: B.daylight.hour, phase: B.daylight.phase, stars: o.stars, torch: o.torch, day: o.day, lights: o.lightCount, tier: B.renderer.quality, lamps: B.lamps.map((l) => +l.node.glow.toFixed(2)), lit: B.lamps.every((l) => l.lit), critters: B.critters, subtitle: document.getElementById("subtitle").textContent, gl: B.renderer.stats }; })()`);
  record("daylight: at 22h the hub is night with stars, every lamp lit and four point lights on the high tier", night.hour === 22 && night.phase === "night" && night.stars === 1 && night.torch === 1 && night.day === 0 && night.lit && night.lamps.every((g) => g > 0.5) && (night.tier !== "high" || night.lights === 4) && night.subtitle.endsWith("night"), JSON.stringify(night));
  ...
  // Space by a tree at night shakes it: leaves fly and fireflies scatter
  const shaken = await b.evaluate(`(() => { const B = window.__ooga; const o = B.props.find((o) => o.prop === "tree" && o.active); const cave = [...B.cavemen.values()].find((c) => c.state === "working" && !c.walk && !c.build); B.crew.control(cave); cave.root.position.x = o.x + 0.8; cave.root.position.z = o.z; return { fireflies: B.critters.fireflies, player: B.crew.player === cave }; })()`);
  await b.sleep(100);
  await b.key(" ");
  await b.sleep(150);
...
const dayCycle = () => withPage("day cycle", hubPage(src, "hour=4.9&daylen=12"), async (b) => {
  // Twelve seconds per day: every phase passes, lamps light at dusk and go out at dawn
  const cycle = await b.evaluate(`new Promise((resolve) => { const B = window.__ooga; const phases = [], lights = {}, toasts = new Set(); let last = null; const t0 = performance.now(); const tick = () => { const p = B.daylight.phase; if (p !== last) { phases.push(p); last = p; } lights[p] = B.renderOpts.lightCount; const t = document.getElementById("toast").textContent; if (t) toasts.add(t); if (phases.length < 7 && performance.now() - t0 < 15000) requestAnimationFrame(tick); else resolve({ phases, lights, toasts: [...toasts], subtitle: document.getElementById("subtitle").textContent, stats: B.stats() }); }; tick(); })`);
  ...
  record("day cycle: point lights are off by day and on at night, with a toast at each phase", cycle.lights.noon === 0 && cycle.lights.morning === 0 && cycle.lights.night === 4 && cycle.lights.midnight === 4 && cycle.toasts.length >= 5, JSON.stringify({ lights: cycle.lights, toasts: cycle.toasts }));
