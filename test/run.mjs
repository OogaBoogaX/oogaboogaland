...
const core = (label, base) => withPage(label, page(base), async (b) => {
  ...
  const bananaEnds = await b.evaluate(`(() => { const brown = "90,58,26", inspect = (geo) => { const point = (i) => [geo.verts[i * 3], geo.verts[i * 3 + 1], geo.verts[i * 3 + 2]], center = (indices) => indices.reduce((sum, i) => { const p = point(i); sum[0] += p[0]; sum[1] += p[1]; sum[2] += p[2]; return sum; }, [0, 0, 0]).map((v) => v / indices.length), cap = (face) => { const pts = face.i.map(point), c = center(face.i), u = pts[1].map((v, i) => v - pts[0][i]), v = pts[2].map((q, i) => q - pts[0][i]), n = [u[1] * v[2] - u[2] * v[1], u[2] * v[0] - u[0] * v[2], u[0] * v[1] - u[1] * v[0]], nl = Math.hypot(...n); n.forEach((q, i) => n[i] = q / nl); return { color: face.color.join(","), radius: pts.reduce((sum, p) => sum + Math.hypot(p[0] - c[0], p[1] - c[1], p[2] - c[2]), 0) / pts.length, planeError: Math.max(...pts.map((p) => Math.abs((p[0] - pts[0][0]) * n[0] + (p[1] - pts[0][1]) * n[1] + (p[2] - pts[0][2]) * n[2]))) }; }, ringRadius = (ring) => { const indices = Array.from({ length: 5 }, (_, i) => ring * 5 + i), c = center(indices); return indices.reduce((sum, i) => { const p = point(i); return sum + Math.hypot(p[0] - c[0], p[1] - c[1], p[2] - c[2]); }, 0) / indices.length; }, caps = geo.faces.filter((face) => face.i.length === 5).map(cap); return { faces: geo.faces.length, caps, brownSides: geo.faces.filter((face) => face.i.length === 4 && face.color.join(",") === brown).length, startRadius: ringRadius(0), bodyRadius: ringRadius(4), unchangedEndRadius: ringRadius(8) }; }; return { loose: inspect(window.BL.models.bananaGeometry()), shell: inspect(window.BL.models.bananaTileGeometry()) }; })()`);
  record(`${label}: every shared banana model flattens only its original brown point while preserving the opposite taper`, [bananaEnds.loose, bananaEnds.shell].every((shape) => shape.faces === 41 && shape.caps.length === 1 && shape.caps[0].color === "90,58,26" && Math.abs(shape.caps[0].radius - 0.032) < 1e-6 && shape.caps[0].planeError < 1e-8 && shape.brownSides === 5 && Math.abs(shape.startRadius - 0.032) < 1e-6 && Math.abs(shape.unchangedEndRadius - 0.012) < 1e-6 && shape.startRadius < shape.bodyRadius * 0.34), JSON.stringify(bananaEnds));
...
const mirrorCave = () => withPage("mirror cave", hubPage(src), async (b) => {
  ...
  record("mirror cave: inward-only lining stops at the vestibule ceiling with no redundant portal header", (!built.roomVisible || built.roomScale === 0) && !built.rainVisible && Math.abs(built.roomFront - 0.48) < 1e-8 && built.declaredFront === 0.48 && built.claddingFrontZ === 0.48 && built.upperPanelFaces === 0 && built.removedHeader.id === "doorwayLintel" && built.removedHeader.backingSurfaceId === "doorwayLintelInner" && built.removedHeader.bounds.join("|") === "-2.9|3|0.48|2.9|3.9|0.48" && built.removedHeader.vestibuleCeiling === 2.98 && built.removedHeader.removedSectionCount === 1 && built.removedHeader.removedPhysicalFaceCount === 1 && !built.removedHeader.physicalBackingRetained && transitionBacking.count === 1 && built.transitionHeaderMinY === 2.98 && built.transitionHeaderMaxY === 3.75 && built.outwardPortalFaces === 0 && built.outwardTransitionFaces === 0 && built.mirrorPlaneZ === 0.5 && built.exteriorRoomVertices === 0 && built.exteriorGap >= 0.019 && built.rimCladdingFaces === 0 && built.transitionCladdingZ === -2.5 && built.guideFaces === 0 && built.blackFaces, JSON.stringify({ roomVisible: built.roomVisible, roomScale: built.roomScale, rainVisible: built.rainVisible, roomFront: built.roomFront, declaredFront: built.declaredFront, claddingFrontZ: built.claddingFrontZ, removedHeader: built.removedHeader, upperPanelFaces: built.upperPanelFaces, transitionBacking: transitionBacking.count, transitionHeaderMinY: built.transitionHeaderMinY, transitionHeaderMaxY: built.transitionHeaderMaxY, outwardPortalFaces: built.outwardPortalFaces, outwardTransitionFaces: built.outwardTransitionFaces, mirrorPlaneZ: built.mirrorPlaneZ, exteriorRoomVertices: built.exteriorRoomVertices, exteriorGap: built.exteriorGap, rimCladdingFaces: built.rimCladdingFaces, transitionCladdingZ: built.transitionCladdingZ, guideFaces: built.guideFaces, blackFaces: built.blackFaces }));
  record("mirror cave: the unused inward rim skin is removed without changing its exterior shell", removedPanel.id === "caveRimInteriorRear" && removedPanel.planeAxis === "z" && removedPanel.planePosition === -0.5 && removedPanel.inwardNormal.join("|") === "0|0|-1" && removedPanel.bounds.join("|") === "-3|0|-0.5|3|3.5|-0.5" && removedPanel.lintelBounds.join("|") === "-2.5|3|-0.5|2.5|3.5|-0.5" && removedPanel.removedFaceCount === 18 && removedPanel.removedLintelFaceCount === 6 && removedPanel.remainingFaceCount === 82 && removedPanel.faceCount === 82 && removedPanel.rearFaces === 0 && removedPanel.frontFaces > 0, JSON.stringify(removedPanel));
  const soffit = await b.evaluate(`(() => { const B = window.__ooga, C = B.mirrorCave, g = C.rim.geometry, removed = B.matrixCave.removedExteriorSoffit, faces = g.faces.map((face) => { const p = face.i.map((i) => [g.verts[i * 3], g.verts[i * 3 + 1], g.verts[i * 3 + 2]]), u = p[1].map((v, i) => v - p[0][i]), v = p[2].map((q, i) => q - p[0][i]); return { min: [Math.min(...p.map((q) => q[0])), Math.min(...p.map((q) => q[1])), Math.min(...p.map((q) => q[2]))], max: [Math.max(...p.map((q) => q[0])), Math.max(...p.map((q) => q[1])), Math.max(...p.map((q) => q[2]))], normalY: u[2] * v[0] - u[0] * v[2], black: face.color.every((channel) => channel === 0) }; }), stone = faces.filter((face) => face.normalY < 0 && face.min[1] === 3 && face.min[0] >= -2.5 && face.max[0] <= 2.5); return { removed: { ...removed, normal: [...removed.normal], bounds: [...removed.bounds], matrixBounds: [...removed.matrixBounds] }, stoneCount: stone.length, stoneBlack: stone.filter((face) => face.black).length, stoneBounds: [Math.min(...stone.map((face) => face.min[2])), Math.max(...stone.map((face) => face.max[2]))], roomChildren: C.room.children.length, hasLiner: "rimLiner" in C, rimSurface: B.matrixCave.surfaceInfo("rimUnderside") }; })()`);
  record("mirror cave: the redundant black soffit overlay is removed and the original stone underside remains", soffit.removed.id === "matrixRimLiner" && soffit.removed.backingSurfaceId === "rimUnderside" && soffit.removed.planeAxis === "y" && soffit.removed.planePosition === 2.995 && soffit.removed.normal.join("|") === "0|-1|0" && soffit.removed.bounds.join("|") === "-2.5|2.995|0.34|2.5|2.995|1.005" && soffit.removed.matrixBounds.join("|") === "-2.5|2.995|0.34|2.5|2.995|0.49" && soffit.removed.portalPlane === 0.5 && soffit.removed.classification === "exterior-frame-underside" && soffit.removed.removedMaterial === "black-interior-lining" && soffit.removed.restoredMaterial === "original-stone-rim" && soffit.removed.removedFaceCount === 1 && soffit.removed.removedSectionCount === 1 && soffit.removed.removedStreamCount === 42 && soffit.removed.removedGlyphCount === 84 && soffit.removed.previousTrainLength === 1 && soffit.removed.previousGapLength === 1 && soffit.removed.previousVisibleTipRatio === 1 && soffit.removed.stoneBackingRetained && soffit.stoneCount === 11 && soffit.stoneBlack === 0 && soffit.stoneBounds.join("|") === "-0.5|0.5" && soffit.roomChildren === 0 && !soffit.hasLiner && soffit.rimSurface === null, JSON.stringify(soffit));
  ...
  await rendered(4);
  const prewarmed = await b.evaluate(`(() => { const B = window.__ooga, a = B.matrixCave.activeSurfaceCounts; return { preloaded: B.matrixCave.preloaded, visible: B.matrixCave.visible, drawEnabled: B.matrixCave.drawEnabled, drawnGlyphs: B.matrixCave.drawnGlyphCount, preloadDistance: B.matrixCave.preloadDistance, prewarmCount: B.matrixCave.prewarmCount, roomVisible: B.mirrorCave.room.visible, roomScale: B.matrixCave.roomScale, records: B.renderer.stats.records, updates: B.matrixCave.updates, activeSurfaces: a.floor + a.ceiling + a.leftWall + a.rightWall + a.backWall + a.doorway, registeredSurfaces: B.matrixCave.surfaceGlyphCount, gaps: B.matrixCave.movingGapCount, y: B.matrixCave.firstGlyphY, portal: B.mirror.portal, surfaceDrawn: B.mirror.surfaceDrawn }; })()`);
  await rendered(8);
  ...
  record("mirror interior: complete effect prewarms while every glyph batch remains draw-disabled outside", prewarmed.preloaded && !prewarmed.visible && !prewarmed.drawEnabled && prewarmed.drawnGlyphs === 0 && prewarmed.preloadDistance === 18 && prewarmed.prewarmCount === 1 && prewarmed.roomVisible && prewarmed.roomScale === 1 && prewarmed.records > 0 && prewarmed.updates > 0 && prewarmed.registeredSurfaces > 9000 && prewarmed.activeSurfaces > 6000 && prewarmed.gaps > 1000 && prewarmedAfter.updates > prewarmed.updates && prewarmedAfter.y !== prewarmed.y && !prewarmed.portal && prewarmed.surfaceDrawn && !prewarmedAfter.portal && prewarmedAfter.surfaceDrawn, JSON.stringify({ before: prewarmed, after: prewarmedAfter }));
  const exteriorMatrixViews = await b.evaluate(`new Promise((resolve) => { const B = window.__ooga, m = B.mirrorCave.mouth, o = B.pilot.orbit, views = [["front", 0, 0.08], ["left-oblique", -0.62, 0.12], ["right-side", 1.08, 0.08], ["elevated", 0.18, 0.52], ["low", -0.18, -0.08]], samples = [], wait = (count, done) => { const start = B.renderedFrames, tick = () => B.renderedFrames >= start + count ? done() : requestAnimationFrame(tick); requestAnimationFrame(tick); }, pose = (view) => { const localZ = 0.45, target = { x: m.x + Math.sin(m.ry) * localZ, y: m.floorY + 1.55, z: m.z + Math.cos(m.ry) * localZ }; o.target = target; o.tx = target.x; o.ty = target.y; o.tz = target.z; o.yaw = o.tYaw = m.ry + view[1]; o.pitch = o.tPitch = view[2]; o.dist = o.tDist = 6; B.pilot.update(0.1); }, next = (i) => { if (i === views.length) return resolve({ samples, facings: { ...B.matrixCave.surfaceFacings }, maxLocalZ: { ...B.matrixCave.surfaceMaxLocalZ }, portalZ: B.matrixCave.portal.opening.planeZ, clearance: B.matrixCave.portalClearance }); pose(views[i]); wait(4, () => { samples.push({ name: views[i][0], inside: B.matrixCave.inside, visible: B.matrixCave.visible, drawEnabled: B.matrixCave.drawEnabled, drawn: B.matrixCave.drawnGlyphCount, batchDrawn: B.matrixCave.batchDrawCount, active: B.matrixCave.activeGlyphCount, suppressed: B.renderer.stats.suppressed }); next(i + 1); }); }; B.matrixCave.viewApproach(); wait(3, () => next(0)); })`);
  const exteriorFacingNames = ["transitionHeader", "transitionReturns"];
  record("mirror exterior: prewarmed Matrix surfaces stay absent from frontal, oblique, side, elevated and low views", exteriorMatrixViews.samples.length === 5 && exteriorMatrixViews.samples.every((sample) => !sample.inside && !sample.visible && !sample.drawEnabled && sample.drawn === 0 && sample.batchDrawn === 0 && sample.active > 6000 && sample.suppressed >= sample.active), JSON.stringify(exteriorMatrixViews.samples));
  record("mirror exterior: retained entrance sheets face inward and every complete glyph bound remains behind the portal", exteriorFacingNames.every((name) => exteriorMatrixViews.facings[name] === -1) && Object.entries(exteriorMatrixViews.facings).filter(([name]) => !exteriorFacingNames.includes(name) && name !== "rimUnderside").every(([, facing]) => facing === 1) && exteriorMatrixViews.facings.rimUnderside === 0 && exteriorMatrixViews.maxLocalZ.rimUnderside === null && Object.values(exteriorMatrixViews.maxLocalZ).filter(Number.isFinite).every((z) => z < exteriorMatrixViews.portalZ) && exteriorMatrixViews.clearance >= 0.0099, JSON.stringify({ facings: exteriorMatrixViews.facings, maxLocalZ: exteriorMatrixViews.maxLocalZ, portalZ: exteriorMatrixViews.portalZ, clearance: exteriorMatrixViews.clearance }));
  ...
  const soffitInside = await b.evaluate(`(() => { const B = window.__ooga; return { portal: B.matrixCave.inside, hasLiner: "rimLiner" in B.mirrorCave, roomChildren: B.mirrorCave.room.children.length, surface: B.matrixCave.surfaceInfo("rimUnderside"), active: B.matrixCave.activeSurfaceCounts.rimUnderside }; })()`);
  await rendered(16);
  const matrixAfter = await b.evaluate(`(() => { const B = window.__ooga; return { y: B.matrixCave.firstGlyphY, updates: B.matrixCave.updates, version: B.matrixCave.glyphVersion, hash: B.matrixCave.mutationHash, active: B.matrixCave.activeGlyphCount, gaps: B.matrixCave.movingGapCount, drawn: B.matrixCave.drawnGlyphCount, drawEnabled: B.matrixCave.drawEnabled, allocations: B.matrixCave.allocationCount, rebuilds: B.matrixCave.rebuildCount, passes: B.mirror.reflectionPassCount, surfaceDrawn: B.mirror.surfaceDrawn, reason: B.mirror.skipReason, resources: B.mirror.resources, ceilingEntranceBand: B.matrixCave.ceilingEntranceBand }; })()`);
  record("mirror interior: the original dense free rain and bright falling tips remain animated", matrixBefore.scene === "hub" && matrixBefore.cameraInside && matrixBefore.matrix.visible && matrixBefore.matrix.streamCount === 96 && matrixBefore.matrix.hangingStreamCount === 48 && matrixBefore.matrix.entranceStreamCount === 32 && matrixBefore.matrix.wallStreamCount === 48 && matrixBefore.matrix.rainGlyphCount === 1344 && matrixBefore.matrix.activeSurfaceCounts.freeRain > 700 && matrixBefore.matrix.brightTipCount > 600 && matrixAfter.updates > matrixBefore.matrix.updates && matrixAfter.y !== matrixBefore.matrix.firstGlyphY, JSON.stringify({ before: matrixBefore, after: matrixAfter }));
  const surfaces = matrixBefore.matrix.surfaceCounts, streams = matrixBefore.matrix.surfaceStreams, layout = matrixBefore.matrix.surfaceLayout;
  const namedSurfaceCounts = [surfaces.floor, surfaces.mainCeiling, surfaces.vestibuleCeiling, surfaces.mainLeftWall, surfaces.mainRightWall, surfaces.vestibuleLeftWall, surfaces.vestibuleRightWall, surfaces.backWall, surfaces.transitionHeader, surfaces.transitionReturns];
  record("mirror interior: tightly packed code covers every retained chamber surface in its required direction", matrixBefore.matrix.surfaceGlyphCount > 9000 && matrixBefore.matrix.surfacePitch === 0.12 && matrixBefore.matrix.surfaceGap === 0.13 && namedSurfaceCounts.every((count) => count > 0) && surfaces.doorwayLintel === 0 && surfaces.doorwayJambs === 0 && surfaces.rimUnderside === 0 && surfaces.leftWall === surfaces.rightWall && [streams.floor, streams.ceiling, streams.leftWall, streams.rightWall, streams.backWall, streams.doorway].every((count) => count > 0) && layout.floor.plane === "horizontal" && layout.floor.flow === "entrance-to-back" && layout.ceiling.flow === "entrance-to-back" && layout.leftWall.plane === "vertical" && layout.leftWall.flow === "down" && layout.rightWall.flow === "down" && layout.backWall.flow === "down", JSON.stringify({ surfaces, streams, layout }));
  const surfaceAudit = await b.evaluate(`(() => { const M = window.__ooga.matrixCave, names = ["floor", "mainCeiling", "vestibuleCeiling", "mainLeftWall", "mainRightWall", "vestibuleLeftWall", "vestibuleRightWall", "backWall", "transitionHeader", "transitionReturns"]; return { details: names.map((name) => M.surfaceInfo(name)), removedSoffit: M.surfaceInfo("rimUnderside"), soffit: { ...M.removedExteriorSoffit }, removedLintel: M.surfaceInfo("doorwayLintel"), removedHeader: { ...M.removedExteriorHeader }, removedJambs: M.surfaceInfo("doorwayJambs"), removed: { ...M.removedExteriorMatrixSurface }, oldMainFloor: M.surfaceInfo("mainFloor"), oldVestibuleFloor: M.surfaceInfo("vestibuleFloor"), floorSectionCount: M.floorSectionCount, floorBackingCount: M.floorBackingCount, floorBacking: M.floorBackingSurfaceIdentifier, floorStreams: M.floorStreamCount, floorGlyphs: M.floorGlyphCount, surfaceSections: M.surfaceSectionCount, surfaceStreams: M.surfaceStreamCount, surfaceGlyphs: M.surfaceGlyphCount, capacity: M.capacity, bufferBytes: M.bufferBytes, registryBytes: M.registryBytes, invalidBackings: M.invalidBackingSurfaceCount, portalCrossings: M.portalCrossingSurfaceCount, physicalPortalCrossings: M.physicalInteriorPortalCrossingFaceCount, metadataBytes: M.surfaceMetadataBytes, pitch: M.surfacePitch }; })()`);
  record("mirror interior: every retained glyph layer is aligned to an inward-facing physical backing surface", surfaceAudit.details.length === 10 && surfaceAudit.details.every((detail) => detail && detail.backingSurfaceIds.length === detail.partCount && detail.inwardFacing && detail.fullyBacked && !detail.crossesPortalPlane && detail.maximumBackingPlaneAlignmentError < 1e-8 && detail.minimumGlyphToBackingClearance >= 0.0099 && detail.uncoveredEdgeMargin < surfaceAudit.pitch * 0.5), JSON.stringify(surfaceAudit));
  const floorSurface = surfaceAudit.details.find((detail) => detail.id === "floor");
  record("mirror interior: one full-width floor descriptor reuses one backing from the back wall through the portal", surfaceAudit.oldMainFloor === null && surfaceAudit.oldVestibuleFloor === null && surfaceAudit.floorSectionCount === 1 && surfaceAudit.floorBackingCount === 1 && surfaceAudit.floorBacking === "chamberFloorTop" && floorSurface.partCount === 1 && floorSurface.backingSurfaceIds.join("|") === "chamberFloorTop" && floorSurface.planeAxis === "y" && floorSurface.planePosition === 0.02 && floorSurface.inwardNormal.join("|") === "0|1|0" && floorSurface.coverageBounds.join("|") === "-2.82|0.02|-6.13|2.82|0.02|0.48", JSON.stringify({ floorSurface, floorSectionCount: surfaceAudit.floorSectionCount, floorBackingCount: surfaceAudit.floorBackingCount, floorBacking: surfaceAudit.floorBacking }));
  record("mirror interior: every floor stream shares one uninterrupted animation domain across the former join", floorSurface.streamCount === surfaceAudit.floorStreams && floorSurface.glyphCount === surfaceAudit.floorGlyphs && floorSurface.streamCount > 0 && floorSurface.minimumCharactersPerStream === floorSurface.maximumCharactersPerStream && floorSurface.glyphCount === floorSurface.streamCount * floorSurface.minimumCharactersPerStream && floorSurface.flowMin < -2.5 && floorSurface.flowMax > -2.5 && floorSurface.glyphBounds[0] >= floorSurface.coverageBounds[0] && floorSurface.glyphBounds[3] <= floorSurface.coverageBounds[3] && floorSurface.glyphBounds[1] > floorSurface.coverageBounds[1] && floorSurface.glyphBounds[2] >= floorSurface.coverageBounds[2] && floorSurface.glyphBounds[5] <= floorSurface.coverageBounds[5], JSON.stringify(floorSurface));
  const vestibuleLeft = surfaceAudit.details.find((detail) => detail.id === "vestibuleLeftWall"), vestibuleRight = surfaceAudit.details.find((detail) => detail.id === "vestibuleRightWall");
  const floorViews = await b.evaluate(`new Promise((resolve) => { const B = window.__ooga, m = B.mirrorCave.mouth, o = B.pilot.orbit, poses = [{ name: "front", yaw: 0, pitch: 0.08 }, { name: "left", yaw: -0.55, pitch: 0.08 }, { name: "right", yaw: 0.55, pitch: 0.08 }, { name: "elevated", yaw: 0, pitch: 0.32 }, { name: "low", yaw: 0, pitch: -0.18 }], samples = [], next = (i) => { if (i === poses.length) { B.matrixCave.viewInside(false); return resolve(samples); } const p = poses[i]; o.yaw = o.tYaw = m.ry + p.yaw; o.pitch = o.tPitch = p.pitch; B.pilot.update(0.1); const start = B.renderedFrames, tick = () => { if (B.renderedFrames >= start + 3) { const floor = B.matrixCave.surfaceInfo("floor"); samples.push({ name: p.name, inside: B.matrixCave.inside, visible: B.matrixCave.visible, drawn: B.matrixCave.drawnGlyphCount, activeFloor: B.matrixCave.activeSurfaceCounts.floor, sections: B.matrixCave.floorSectionCount, backing: B.matrixCave.floorBackingSurfaceIdentifier, bounds: floor.coverageBounds.join("|") }); next(i + 1); } else requestAnimationFrame(tick); }; requestAnimationFrame(tick); }; next(0); })`);
  record("mirror interior: the seamless floor remains one visible camera-independent surface from frontal, side, oblique, elevated and low views", floorSurface.coverageBounds[0] < vestibuleLeft.planePosition && floorSurface.coverageBounds[3] > vestibuleRight.planePosition && floorViews.length === 5 && floorViews.every((sample) => sample.inside && sample.visible && sample.drawn > 0 && sample.activeFloor > 0 && sample.sections === 1 && sample.backing === "chamberFloorTop" && sample.bounds === "-2.82|0.02|-6.13|2.82|0.02|0.48"), JSON.stringify({ vestibule: [vestibuleLeft.planePosition, vestibuleRight.planePosition], floorViews }));
  const insetHeader = surfaceAudit.details.find((detail) => detail.id === "transitionHeader"), transitionReturns = surfaceAudit.details.find((detail) => detail.id === "transitionReturns");
  record("mirror interior: the recessed ceiling riser and entrance returns use their exact bounded backing planes", insetHeader.backingSurfaceIds.join("|") === "recessedCeilingRiser" && insetHeader.planeAxis === "z" && insetHeader.planePosition === -2.5 && insetHeader.inwardNormal.join("|") === "0|0|-1" && insetHeader.coverageBounds[1] === 2.98 && insetHeader.coverageBounds[4] === 3.75 && insetHeader.glyphBounds[1] >= insetHeader.coverageBounds[1] && insetHeader.glyphBounds[4] <= insetHeader.coverageBounds[4] && insetHeader.glyphBounds[5] < insetHeader.planePosition && transitionReturns.partCount === 2 && transitionReturns.backingSurfaceIds.join("|") === "transitionLeftReturn|transitionRightReturn", JSON.stringify({ insetHeader, transitionReturns }));
  record("mirror entrance: the redundant portal lintel panel and its Matrix descriptor are both absent", surfaceAudit.removedLintel === null && surfaceAudit.removedHeader.id === "doorwayLintel" && surfaceAudit.removedHeader.backingSurfaceId === "doorwayLintelInner" && surfaceAudit.removedHeader.bounds.join("|") === "-2.9|3|0.48|2.9|3.9|0.48" && surfaceAudit.removedHeader.vestibuleCeiling === 2.98 && surfaceAudit.removedHeader.removedSectionCount === 1 && surfaceAudit.removedHeader.removedPhysicalFaceCount === 1 && !surfaceAudit.removedHeader.physicalBackingRetained, JSON.stringify({ removedLintel: surfaceAudit.removedLintel, removedHeader: surfaceAudit.removedHeader }));
  record("mirror exterior: the redundant jamb overlays are absent while their black interior backing remains", surfaceAudit.removedJambs === null && surfaceAudit.removed.id === "doorwayJambs" && surfaceAudit.removed.removedSectionCount === 2 && surfaceAudit.removed.physicalBackingRetained && surfaceAudit.removed.backingSurfaceIds.join("|") === "doorwayLeftJambInner|doorwayRightJambInner" && surfaceAudit.removed.partBounds.length === 2, JSON.stringify({ removedJambs: surfaceAudit.removedJambs, removed: surfaceAudit.removed }));
  record("mirror interior: every retained Matrix entrance surface remains inward-facing, backed and behind the portal", surfaceAudit.surfaceSections === 11 && surfaceAudit.surfaceStreams === 345 && surfaceAudit.surfaceGlyphs === 9222 && surfaceAudit.capacity === 10568 && surfaceAudit.bufferBytes === 845440 && surfaceAudit.registryBytes === 697036 && surfaceAudit.metadataBytes === 1408 && surfaceAudit.invalidBackings === 0 && surfaceAudit.portalCrossings === 0 && surfaceAudit.physicalPortalCrossings === 0 && surfaceAudit.removedSoffit === null && surfaceAudit.details.every((detail) => detail.inwardFacing && detail.fullyBacked && !detail.crossesPortalPlane), JSON.stringify({ counts: { sections: surfaceAudit.surfaceSections, streams: surfaceAudit.surfaceStreams, glyphs: surfaceAudit.surfaceGlyphs, capacity: surfaceAudit.capacity, bufferBytes: surfaceAudit.bufferBytes, registryBytes: surfaceAudit.registryBytes, metadataBytes: surfaceAudit.metadataBytes }, invalidBackings: surfaceAudit.invalidBackings, portalCrossings: surfaceAudit.portalCrossings, physicalPortalCrossings: surfaceAudit.physicalPortalCrossings, entrance: surfaceAudit.details.filter((detail) => ["transitionHeader", "transitionReturns"].includes(detail.id)), soffit: surfaceAudit.soffit }));
  record("mirror interior: the doorway stays open and all retained surface code remains behind the mirror plane", !layout.doorway && surfaceAudit.removedLintel === null && matrixBefore.matrix.surfaceCounts.doorwayLintel === 0 && layout.floor.maxZ < matrixBefore.matrix.portal.opening.planeZ && matrixBefore.matrix.surfaceCounts.doorway > 0, JSON.stringify({ doorway: layout.doorway, removedLintel: surfaceAudit.removedLintel, floor: layout.floor, opening: matrixBefore.matrix.portal.opening }));
  const edgeMargins = [matrixBefore.matrix.surfaceEdgeMargins.floor, matrixBefore.matrix.surfaceEdgeMargins.mainCeiling, matrixBefore.matrix.surfaceEdgeMargins.vestibuleCeiling, matrixBefore.matrix.surfaceEdgeMargins.mainLeftWall, matrixBefore.matrix.surfaceEdgeMargins.mainRightWall, matrixBefore.matrix.surfaceEdgeMargins.vestibuleLeftWall, matrixBefore.matrix.surfaceEdgeMargins.vestibuleRightWall, matrixBefore.matrix.surfaceEdgeMargins.backWall, matrixBefore.matrix.surfaceEdgeMargins.transitionHeader, matrixBefore.matrix.surfaceEdgeMargins.transitionReturns];
  record("mirror interior: every retained surface grid reaches its shared seams", edgeMargins.length === 10 && edgeMargins.every((margin) => margin < matrixBefore.matrix.surfacePitch * 0.5), JSON.stringify({ edgeMargins, halfPitch: matrixBefore.matrix.surfacePitch * 0.5 }));
  const ceilingBands = [matrixBefore.matrix.ceilingEntranceBand, matrixAfter.ceilingEntranceBand];
  record("mirror ceiling: entrance streams use varied floor-like phases instead of a synchronized bright band", ceilingBands.every((band) => band.sampledStreamCount === 41 && band.phaseBucketCount >= 14 && band.phaseBucketCapacity === 16 && band.gapLengthDiversity >= 4 && band.streamBrightnessMax - band.streamBrightnessMin > 0.3 && Math.abs(band.streamBrightnessMin - band.floorStreamBrightnessMin) < 0.08 && Math.abs(band.streamBrightnessMax - band.floorStreamBrightnessMax) < 0.08 && band.sampledGlyphCount > 100 && band.visibleGlyphCount > 60 && band.gapGlyphCount > 10 && band.brightTipCount > 0 && band.brightTipCount / band.visibleGlyphCount < 0.4 && band.visibleBrightnessMax - band.visibleBrightnessMin > 0.3), JSON.stringify(ceilingBands));
  record("mirror interior: deterministic stream brightness, train lengths and moving blank gaps vary within bounds", matrixBefore.matrix.minBrightness >= 0.58 && matrixBefore.matrix.maxBrightness <= 0.94 && matrixBefore.matrix.maxBrightness - matrixBefore.matrix.minBrightness > 0.3 && matrixBefore.matrix.minTrainLength >= 1 && matrixBefore.matrix.maxTrainLength === 14 && matrixBefore.matrix.minGapLength >= 1 && matrixBefore.matrix.maxGapLength >= 6 && matrixBefore.matrix.movingGapCount > 1000 && matrixBefore.matrix.maxLocalZ < matrixBefore.matrix.portal.opening.planeZ && matrixBefore.matrix.portalClearance >= 0.0099, JSON.stringify({ brightness: [matrixBefore.matrix.minBrightness, matrixBefore.matrix.maxBrightness], trains: [matrixBefore.matrix.minTrainLength, matrixBefore.matrix.maxTrainLength], gaps: [matrixBefore.matrix.minGapLength, matrixBefore.matrix.maxGapLength], movingGaps: matrixBefore.matrix.movingGapCount, maxLocalZ: matrixBefore.matrix.maxLocalZ, clearance: matrixBefore.matrix.portalClearance }));
  record("mirror interior: glyph identities mutate at ten hertz with moving gaps and no resource rebuild", matrixBefore.matrix.glyphCadenceHz === 10 && matrixAfter.version > matrixBefore.matrix.glyphVersion && matrixAfter.hash !== matrixBefore.matrix.mutationHash && matrixAfter.active > 6000 && matrixAfter.gaps > 1000 && matrixAfter.drawEnabled && matrixAfter.drawn === matrixAfter.active && matrixBefore.matrix.allocationCount === 8 && matrixAfter.allocations === 8 && matrixBefore.matrix.rebuildCount === 1 && matrixAfter.rebuilds === 1 && matrixBefore.matrix.capacity >= matrixBefore.matrix.glyphCount && matrixBefore.matrix.capacity - matrixBefore.matrix.glyphCount < 8, JSON.stringify({ before: { version: matrixBefore.matrix.glyphVersion, hash: matrixBefore.matrix.mutationHash, active: matrixBefore.matrix.activeGlyphCount, capacity: matrixBefore.matrix.capacity, glyphs: matrixBefore.matrix.glyphCount, allocations: matrixBefore.matrix.allocationCount, rebuilds: matrixBefore.matrix.rebuildCount }, after: matrixAfter }));
  const matrixMotion = await b.evaluate(`new Promise((resolve) => { const B = window.__ooga, names = ["floor", "mainCeiling", "mainLeftWall", "backWall", "transitionHeader"], sample = () => Object.fromEntries(names.map((name) => [name, B.matrixCave.sampleMotion(name)])), wait = (count, done) => { const start = B.renderedFrames, tick = () => B.renderedFrames >= start + count ? done() : requestAnimationFrame(tick); requestAnimationFrame(tick); }, before = sample(); B.matrixCave.viewInside(true); wait(10, () => { const turned = sample(); B.matrixCave.viewInside(false); wait(10, () => resolve({ before, turned, restored: sample(), directions: { ...B.matrixCave.surfaceDirections } })); }); })`);
  const forwardDistance = (a, z) => ((a.head - z.head) % a.flowRange + a.flowRange) % a.flowRange;
  const gapDistance = (a, z) => ((a.gap - z.gap) % a.flowRange + a.flowRange) % a.flowRange;
  const motionNames = Object.keys(matrixMotion.before);
  const firstLeg = motionNames.map((name) => ({ name, head: forwardDistance(matrixMotion.before[name], matrixMotion.turned[name]), gap: gapDistance(matrixMotion.before[name], matrixMotion.turned[name]) }));
  const secondLeg = motionNames.map((name) => ({ name, head: forwardDistance(matrixMotion.turned[name], matrixMotion.restored[name]), gap: gapDistance(matrixMotion.turned[name], matrixMotion.restored[name]) }));
  record("mirror interior: each train and its blank gap translate together in the configured direction", [...firstLeg, ...secondLeg].every((sample) => sample.head > 0.01 && Math.abs(sample.head - sample.gap) < 0.002) && Object.values(matrixMotion.directions).every((direction) => direction === -1), JSON.stringify({ firstLeg, secondLeg, directions: matrixMotion.directions }));
  record("mirror interior: leading tips stay ahead of fading trails independent of camera direction", motionNames.every((name) => { const a = matrixMotion.before[name], separation = ((a.gap - a.head) % a.flowRange + a.flowRange) % a.flowRange; return a.direction === -1 && a.leadingGlow > a.secondGlow && a.secondGlow > a.trailingGlow && Math.abs(separation - a.trainLength * 0.13) < 0.002; }), JSON.stringify(matrixMotion));
  const matrixTiers = await b.evaluate(`new Promise((resolve) => { const B = window.__ooga, samples = [], names = ["high", "medium", "low", "high"], surfaceNames = ["floor", "mainCeiling", "vestibuleCeiling", "mainLeftWall", "mainRightWall", "vestibuleLeftWall", "vestibuleRightWall", "backWall", "transitionHeader", "transitionReturns"], sample = () => { const a = B.matrixCave.activeSurfaceCounts; samples.push({ quality: B.matrixCave.quality, density: B.matrixCave.qualityDensity, surfaceActive: a.floor + a.ceiling + a.leftWall + a.rightWall + a.backWall + a.doorway, named: surfaceNames.map((name) => a[name]), removedSoffit: a.rimUnderside, removedLintel: a.doorwayLintel, removedJambs: a.doorwayJambs, allocations: B.matrixCave.allocationCount, rebuilds: B.matrixCave.rebuildCount, buffers: B.matrixCave.bufferCount, bytes: B.matrixCave.bufferBytes }); }, next = (i) => { if (i === names.length) return resolve(samples); B.renderer.setQuality(names[i]); const start = B.renderedFrames, tick = () => { if (B.renderedFrames >= start + 3) { sample(); next(i + 1); } else requestAnimationFrame(tick); }; requestAnimationFrame(tick); }; next(0); })`);
  record("mirror interior: quality tiers retain every required surface while keeping exterior-leaking entrance overlays absent", matrixTiers.map((s) => s.quality).join("|") === "high|medium|low|high" && matrixTiers[0].density === 1 && matrixTiers[1].density === 0.625 && matrixTiers[2].density === 0.375 && matrixTiers[0].surfaceActive > matrixTiers[1].surfaceActive && matrixTiers[1].surfaceActive > matrixTiers[2].surfaceActive && matrixTiers[0].surfaceActive === matrixTiers[3].surfaceActive && matrixTiers.every((s) => s.named.every((count) => count > 0) && s.removedSoffit === 0 && s.removedLintel === 0 && s.removedJambs === 0 && s.allocations === 8 && s.rebuilds === 1 && s.buffers === 8 && s.bytes === matrixTiers[0].bytes), JSON.stringify(matrixTiers));
  ...
  record("mirror entrance: crossing inside does not restore the removed soffit overlay", soffitInside.portal && !soffitInside.hasLiner && soffitInside.roomChildren === 0 && soffitInside.surface === null && soffitInside.active === 0, JSON.stringify(soffitInside));
  await b.evaluate(`window.__ooga.matrixCave.viewInside(true)`);
  await rendered(12);
  const entrancePanelViews = await b.evaluate(`new Promise((resolve) => { const B = window.__ooga, m = B.mirrorCave.mouth, o = B.pilot.orbit, poses = [["front", 0, 0.08], ["left-oblique", -0.62, 0.12], ["right-oblique", 0.62, 0.12], ["side", 1.02, 0.08], ["far-side", 1.5, 0.08], ["elevated", 0.18, 0.38], ["low", -0.18, -0.08]], samples = [], wait = (count, done) => { const start = B.renderedFrames, tick = () => B.renderedFrames >= start + count ? done() : requestAnimationFrame(tick); requestAnimationFrame(tick); }, sample = (name) => { const rim = B.mirrorCave.rim.geometry, panel = B.matrixCave.removedEntrancePanel, removedMatrix = B.matrixCave.removedExteriorMatrixSurface, header = B.matrixCave.removedExteriorHeader, soffit = B.matrixCave.removedExteriorSoffit, room = B.mirrorCave.room.geometry, headerFaces = room.faces.filter((face) => face.i.every((index) => { const x = room.verts[index * 3], y = room.verts[index * 3 + 1], z = room.verts[index * 3 + 2]; return x >= header.bounds[0] && x <= header.bounds[3] && y >= header.bounds[1] && y <= header.bounds[4] && Math.abs(z - header.planePosition) < 1e-8; })).length; samples.push({ name, inside: B.matrixCave.inside, matrixVisible: B.matrixCave.visible, doorwayGlyphs: B.matrixCave.activeSurfaceCounts.doorway, soffitGlyphs: B.matrixCave.activeSurfaceCounts.rimUnderside, lintelGlyphs: B.matrixCave.activeSurfaceCounts.doorwayLintel, jambGlyphs: B.matrixCave.activeSurfaceCounts.doorwayJambs, soffitSurface: B.matrixCave.surfaceInfo("rimUnderside"), lintelSurface: B.matrixCave.surfaceInfo("doorwayLintel"), removedSoffit: soffit.id, hasLiner: "rimLiner" in B.mirrorCave, removedHeader: header.id, headerFaces, removedMatrix: removedMatrix.id, rearFaces: rim.faces.filter((face) => face.i.every((index) => rim.verts[index * 3 + 2] === panel.planePosition)).length, removed: panel.removedFaceCount, records: B.renderer.stats.records }); }, next = (i) => { if (i === poses.length) return sideExit(0); const p = poses[i]; o.yaw = o.tYaw = m.ry + Math.PI + p[1]; o.pitch = o.tPitch = p[2]; B.pilot.update(0.1); wait(3, () => { sample(p[0]); next(i + 1); }); }, localToWorld = (x, y, z) => ({ x: m.x + Math.cos(m.ry) * x + Math.sin(m.ry) * z, y: m.floorY + y, z: m.z - Math.sin(m.ry) * x + Math.cos(m.ry) * z }), sidePoses = [["side-wall-exit-left", -4.6, 3.35, -1.2], ["side-wall-exit-right", 4.6, 3.35, -1.2]], sideExit = (i) => { if (i === sidePoses.length) { B.matrixCave.viewInside(true); return wait(3, () => resolve(samples)); } const p = sidePoses[i], eye = localToWorld(p[1], p[2], p[3]), target = localToWorld(0, 3.35, 0.48), dx = eye.x - target.x, dy = eye.y - target.y, dz = eye.z - target.z, dist = Math.hypot(dx, dy, dz); o.target = target; o.tx = target.x; o.ty = target.y; o.tz = target.z; o.yaw = o.tYaw = Math.atan2(dx, dz); o.pitch = o.tPitch = Math.asin(dy / dist); o.dist = o.tDist = dist; B.pilot.update(0.1); wait(3, () => { sample(p[0]); sideExit(i + 1); }); }; next(0); })`);
  record("mirror entrance: removed soffit, rear stone, lintel, and jamb panels stay absent from every crossing and side-wall rear view", entrancePanelViews.length === 9 && entrancePanelViews.every((sample) => sample.inside && sample.matrixVisible && sample.doorwayGlyphs > 0 && sample.soffitGlyphs === 0 && sample.lintelGlyphs === 0 && sample.jambGlyphs === 0 && sample.soffitSurface === null && sample.lintelSurface === null && sample.removedSoffit === "matrixRimLiner" && !sample.hasLiner && sample.removedHeader === "doorwayLintel" && sample.headerFaces === 0 && sample.removedMatrix === "doorwayJambs" && sample.rearFaces === 0 && sample.removed === 18 && sample.records === entrancePanelViews[0].records), JSON.stringify(entrancePanelViews));
  ...
  const portalCycles = await b.evaluate(`new Promise((resolve) => { const B = window.__ooga, samples = [], base = { records: B.renderer.stats.records, buffers: B.matrixCave.bufferCount, allocations: B.matrixCave.allocationCount, rebuilds: B.matrixCave.rebuildCount }, wait = (count, done) => { const start = B.renderedFrames, tick = () => B.renderedFrames >= start + count ? done() : requestAnimationFrame(tick); requestAnimationFrame(tick); }, cycle = (i) => { if (i === 3) return resolve({ base, samples }); B.matrixCave.viewApproach(); wait(6, () => { samples.push({ side: "outside", inside: B.matrixCave.inside, drawEnabled: B.matrixCave.drawEnabled, drawn: B.matrixCave.drawnGlyphCount, active: B.matrixCave.activeGlyphCount, records: B.renderer.stats.records, resources: B.mirror.resources }); B.matrixCave.viewInside(false); wait(6, () => { samples.push({ side: "inside", inside: B.matrixCave.inside, drawEnabled: B.matrixCave.drawEnabled, drawn: B.matrixCave.drawnGlyphCount, active: B.matrixCave.activeGlyphCount, records: B.renderer.stats.records, resources: B.mirror.resources }); cycle(i + 1); }); }); }; cycle(0); })`);
  record("mirror interior: repeated crossings switch draw state in the crossing frame without growing resources", portalCycles.samples.filter((sample) => sample.side === "outside").every((sample) => !sample.inside && !sample.drawEnabled && sample.drawn === 0) && portalCycles.samples.filter((sample) => sample.side === "inside").every((sample) => sample.inside && sample.drawEnabled && sample.drawn === sample.active && sample.drawn > 6000) && portalCycles.samples.every((sample) => sample.records === portalCycles.base.records && sample.resources === 6) && portalCycles.base.buffers === 8 && portalCycles.base.allocations === 8 && portalCycles.base.rebuilds === 1, JSON.stringify(portalCycles));
  ...
  const matrixPerf = await b.evaluate(`new Promise((resolve) => { const B = window.__ooga; B.renderer.setQuality("high"); B.setPileLevel(1000000); B.matrixCave.viewInside(false); const startFrame = B.renderedFrames, start = performance.now(), tick = () => { const frames = B.renderedFrames - startFrame, seconds = (performance.now() - start) / 1000; if (frames >= 120 || seconds > 6) resolve({ fps: frames / seconds, frames, shown: B.shown, active: B.matrixCave.activeGlyphCount, buffers: B.matrixCave.bufferCount, bytes: B.matrixCave.bufferBytes, allocations: B.matrixCave.allocationCount, rebuilds: B.matrixCave.rebuildCount, records: B.renderer.stats.records }); else requestAnimationFrame(tick); }; requestAnimationFrame(tick); })`);
  record("mirror interior: full surface code and the million-banana pile remain at least 50 FPS", matrixPerf.fps >= 50 && matrixPerf.frames >= 120 && matrixPerf.shown === 1000000 && matrixPerf.active > 6000 && matrixPerf.buffers === 8 && matrixPerf.bytes < 900000 && matrixPerf.allocations === 8 && matrixPerf.rebuilds === 1, JSON.stringify(matrixPerf));
...
const mirrorCanvas = () => withPage("mirror canvas fallback", hubPage(src, "canvas2d=1&bananas=1000000&hour=22"), async (b) => {
  ...
  const canvasPanel = await b.evaluate(`(() => { const B = window.__ooga, rim = B.mirrorCave.rim.geometry, panel = B.matrixCave.removedEntrancePanel, room = B.mirrorCave.room.geometry, header = B.matrixCave.removedExteriorHeader, soffit = B.matrixCave.removedExteriorSoffit; return { id: panel.id, removed: panel.removedFaceCount, remaining: panel.remainingFaceCount, rearFaces: rim.faces.filter((face) => face.i.every((index) => rim.verts[index * 3 + 2] === panel.planePosition)).length, removedHeader: header.id, headerFaces: room.faces.filter((face) => face.i.every((index) => { const x = room.verts[index * 3], y = room.verts[index * 3 + 1], z = room.verts[index * 3 + 2]; return x >= header.bounds[0] && x <= header.bounds[3] && y >= header.bounds[1] && y <= header.bounds[4] && Math.abs(z - header.planePosition) < 1e-8; })).length, lintelSurface: B.matrixCave.surfaceInfo("doorwayLintel"), removedSoffit: soffit.id, soffitSurface: B.matrixCave.surfaceInfo("rimUnderside"), hasLiner: "rimLiner" in B.mirrorCave }; })()`);
  record("mirror canvas fallback: unused inward entrance skins remain absent", canvasPanel.id === "caveRimInteriorRear" && canvasPanel.removed === 18 && canvasPanel.remaining === 82 && canvasPanel.rearFaces === 0 && canvasPanel.removedHeader === "doorwayLintel" && canvasPanel.headerFaces === 0 && canvasPanel.lintelSurface === null && canvasPanel.removedSoffit === "matrixRimLiner" && canvasPanel.soffitSurface === null && !canvasPanel.hasLiner, JSON.stringify(canvasPanel));
  ...
  await b.evaluate(`window.__ooga.matrixCave.viewApproach()`);
  await b.sleep(150);
  const canvasExterior = await b.evaluate(`(() => { const B = window.__ooga; return { inside: B.matrixCave.inside, visible: B.matrixCave.visible, drawn: B.matrixCave.drawnGlyphCount, batchDrawn: B.matrixCave.batchDrawCount, active: B.matrixCave.activeGlyphCount, suppressed: B.renderer.stats.suppressed }; })()`);
  record("mirror exterior: Canvas fallback suppresses every prewarmed Matrix glyph outside", !canvasExterior.inside && !canvasExterior.visible && canvasExterior.drawn === 0 && canvasExterior.batchDrawn === 0 && canvasExterior.active > 1000 && canvasExterior.suppressed >= canvasExterior.active, JSON.stringify(canvasExterior));
  const matrix = await b.evaluate(`new Promise((resolve) => { const B = window.__ooga, start = B.renderedFrames; B.matrixCave.viewInside(true); const tick = () => { if (B.renderedFrames >= start + 8) resolve({ scene: B.scene, kind: B.renderer.kind, active: B.mirror.active, portal: B.mirror.portal, surfaceDrawn: B.mirror.surfaceDrawn, faux: B.mirror.faux, resources: B.mirror.resources, streams: B.matrixCave.streamCount, hanging: B.matrixCave.hangingStreamCount, entrance: B.matrixCave.entranceStreamCount, walls: B.matrixCave.wallStreamCount, rainGlyphs: B.matrixCave.rainGlyphCount, surfaceGlyphs: B.matrixCave.surfaceGlyphCount, activeGlyphs: B.matrixCave.activeGlyphCount, surfaces: { ...B.matrixCave.activeSurfaceCounts }, tips: B.matrixCave.brightTipCount, capacity: B.matrixCave.capacity, quality: B.matrixCave.quality, density: B.matrixCave.qualityDensity, buffers: B.matrixCave.bufferCount, bytes: B.matrixCave.bufferBytes, allocations: B.matrixCave.allocationCount, rebuilds: B.matrixCave.rebuildCount, visible: B.matrixCave.visible, inside: B.matrixCave.contains(B.camera.position.x, B.camera.position.z), updates: B.matrixCave.updates }); else requestAnimationFrame(tick); }; requestAnimationFrame(tick); })`);
  const canvasNamed = [matrix.surfaces.floor, matrix.surfaces.mainCeiling, matrix.surfaces.vestibuleCeiling, matrix.surfaces.mainLeftWall, matrix.surfaces.mainRightWall, matrix.surfaces.vestibuleLeftWall, matrix.surfaces.vestibuleRightWall, matrix.surfaces.backWall, matrix.surfaces.transitionHeader, matrix.surfaces.transitionReturns];
  record("mirror interior: Canvas fallback draws every retained surface without exterior-leaking entrance overlays", matrix.scene === "hub" && matrix.kind === "canvas2d" && matrix.active && matrix.portal && !matrix.surfaceDrawn && matrix.faux && matrix.resources === 0 && matrix.streams === 32 && matrix.hanging === 18 && matrix.entrance === 12 && matrix.walls === 14 && matrix.rainGlyphs === 288 && matrix.surfaceGlyphs > 9000 && matrix.activeGlyphs > 1000 && canvasNamed.every((count) => count > 0) && matrix.surfaces.rimUnderside === 0 && matrix.surfaces.doorwayLintel === 0 && matrix.surfaces.doorwayJambs === 0 && matrix.tips > 80 && matrix.capacity >= matrix.activeGlyphs && matrix.capacity < matrix.rainGlyphs + matrix.surfaceGlyphs && matrix.quality === "canvas2d" && matrix.density === 0.125 && matrix.buffers === 8 && matrix.bytes < 200000 && matrix.allocations === 8 && matrix.rebuilds === 1 && matrix.visible && matrix.inside && matrix.updates > 0, JSON.stringify(matrix));
  const canvasMotion = await b.evaluate(`new Promise((resolve) => { const B = window.__ooga, before = B.matrixCave.sampleMotion("floor"), start = B.renderedFrames, tick = () => { if (B.renderedFrames >= start + 10) resolve({ before, after: B.matrixCave.sampleMotion("floor") }); else requestAnimationFrame(tick); }; requestAnimationFrame(tick); })`);
...
const hubRace = () => withPage("hub race route", hubPage(src), async (b) => {
  ...
  await b.sleep(1200);
  const mouth = await b.evaluate(`(() => { const B = window.__ooga; const m = B.mouths.find((m) => m.id === "c9"); const p = B.project(m.x, 2, m.z); const hit = B.input.pick(p.x, p.y); return { x: Math.round(p.x), y: Math.round(p.y), kind: hit && hit.owner.kind, slot: hit && hit.owner.slot && hit.owner.slot.id, status: hit && hit.owner.slot && hit.owner.slot.status, label: B.labels.some((l) => l.text === "Ooga Rally"), torches: B.entranceLights.filter((l) => l.caveId === "c9").length, wheels: (() => { let n = 0; const wheel = window.BL.raceModels.kartWheel(); const walk = (node) => { if (node.geometry === wheel) n++; for (const c of node.children) walk(c); }; walk(window.BL.scenes.hub.root); return n; })(), shelves: (() => { let n = 0; const shelf = window.BL.hubModels.caveShelves(); const walk = (node) => { if (node.geometry === shelf) n++; for (const c of node.children) walk(c); }; walk(window.BL.scenes.hub.root); return n; })() }; })()`);
  await b.click(mouth.x, mouth.y);
  await b.sleep(1600);
  ...
  record("hub race route: the Ooga Rally cave is lit, signed and tapping it enters the garage", mouth.kind === "cave" && mouth.slot === "c9" && mouth.status === "open" && mouth.label && mouth.torches === 3 && mouth.wheels === 7 && mouth.shelves === 2 && entered.scene === "race" && entered.phase === "garage" && entered.garage, JSON.stringify({ ...mouth, ...entered }));
  await b.key("Escape");
  await b.sleep(900);
  ...
  await b.evaluate(`(() => { const B = window.__ooga; const cave = [...B.cavemen.values()].find((c) => c.state === "working" && !c.walk && !c.build); const m = B.mouths.find((m) => m.id === "c9"); B.crew.control(cave); cave.root.position.x = m.apron.x; cave.root.position.z = m.apron.z; cave.root.position.y = cave.baseY; cave.hop = 0; cave.root.rotation.y = Math.PI - m.angle + Math.PI; B.pilot.orbit.tYaw = B.pilot.orbit.yaw = m.angle + Math.PI; })()`);
  await b.sleep(300);
  await b.send("Input.dispatchKeyEvent", { type: "keyDown", key: "w", text: "w" });
