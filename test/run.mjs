...
const mirrorCave = ["mirror cave", async (b) => {
  ...
  record("mirror world: crossing reveals the complete pile-centered Matrix world immediately", matrixWorldEarly.active && matrixWorldEarly.radius === matrixWorldEarly.maxRadius && matrixWorldEarly.direction === 0 && matrixWorldEarly.speed === 72 && matrixWorldEarly.maxRadius >= 38 && matrixWorldEarly.density === 1 && matrixWorldEarly.pile && matrixWorldEarly.meadow && matrixWorldEarly.rim && matrixWorldEarly.cave && matrixWorldEarly.distances.join("|") === "0|12|22|27", JSON.stringify(matrixWorldEarly));
...
const matrixRain = (backend) => withPage(`matrix falling rain ${backend}`, hubPage(src, backend === "canvas2d" ? "canvas2d=1" : ""), async (b) => {
  ...
  record(`${label}: entry activates every cave and gate rain in one update and exit begins a bounded retreat`, !r.inactive.active && r.inactive.radius === 0 && r.inactive.gate.count === 0 && r.inactive.caves.every((c) => c.count === 0 && c.drawn === 0) && r.before.inside && r.before.active && r.before.direction === 0 && r.before.radius === 38 && r.before.gate.count > 0 && r.before.gate.updates > r.inactive.gate.updates && r.before.caves.every((c, i) => c.count > 0 && c.drawn === c.count && c.updates > r.inactive.caves[i].updates) && !r.reversed.inside && r.reversed.direction === -1 && r.reversed.radius < r.before.radius && r.reversed.gate.count > 0 && r.reversed.caves.every((c) => c.farthest <= r.reversed.radius + 0.16001 && c.missing === 0 && c.unwanted === 0), JSON.stringify({ inactive: { radius: r.inactive.radius, gate: r.inactive.gate.count, caves: r.inactive.caves.map((c) => c.count) }, entered: { radius: r.before.radius, direction: r.before.direction, gate: r.before.gate.count, caves: r.before.caves.map((c) => c.count) }, reversed: { radius: r.reversed.radius, direction: r.reversed.direction, gate: r.reversed.gate.count, caves: r.reversed.caves.map((c) => ({ id: c.id, count: c.count, farthest: c.farthest })) } }));
...
const matrixWave = (backend) => [`matrix reversible wave ${backend}`, async (b) => {
  ...
  record(`${label}: the entrance crossing activates the complete Matrix world immediately and populates every cave in the same update`, r.start.radius === 0 && !r.start.active && r.entryCrossing.inside && r.entryCrossing.active && r.entryCrossing.radius === r.maxRadius && r.entryCrossing.direction === 0 && r.entryCrossing.nodePortal && r.entered.portal && r.entered.radius === r.maxRadius && r.entered.direction === 0 && r.entered.caves.every((c, i) => c.updates > r.start.caves[i].updates && c.revealed > 0 && c.count > 0 && c.drawn > 0), JSON.stringify({ start: r.start, crossing: r.entryCrossing, entered: r.entered }));
  record(`${label}: removal retains its 72-unit speed while reentry restores full coverage without replaying the outward wave`, r.speed === 72 && r.retreatSpeed === 72 && !r.exit.inside && r.exit.direction === -1 && r.exit.active && slope(r.beforeExit, r.exit, -r.retreatSpeed) && slope(r.exit, r.reverse, -r.retreatSpeed) && r.partialReentryCrossing.inside && r.partialReentryCrossing.radius === r.maxRadius && r.partialReentryCrossing.direction === 0 && r.reentry.radius === r.maxRadius && r.reentry.direction === 0 && r.resumed.radius === r.maxRadius && r.resumed.direction === 0, JSON.stringify({ speed: r.speed, retreatSpeed: r.retreatSpeed, beforeExit: r.beforeExit.radius, exit: r.exit.radius, reverse: r.reverse.radius, crossing: r.partialReentryCrossing.radius, reentry: r.reentry.radius, resumed: r.resumed.radius }));
  ...
  record(`${label}: immediate reentry opens the mirror and restores the complete glyph world before the next frame`, immediateReentries.every(([, crossing, reentry, mirror]) => crossing.inside && crossing.nodePortal && crossing.radius === r.maxRadius && crossing.direction === 0 && reentry.inside && reentry.portal && reentry.nodePortal && reentry.radius === r.maxRadius && reentry.direction === 0 && mirror.drawn && mirror.portal && !mirror.surfaceDrawn && mirror.captures === 0), JSON.stringify(immediateReentries.map(([before, crossing, reentry, mirror]) => ({ before: before.radius, crossing: crossing.radius, reentry: reentry.radius, portal: reentry.portal, drawn: mirror.surfaceDrawn, captures: mirror.captures }))));
  ...
  record(`${label}: instant activation fills all seven bounded cave layers through their farthest registered surfaces`, r.entered.radius === r.maxRadius && r.entered.caves.length === 7 && r.entered.caves.every((c) => c.minimum < r.maxRadius && c.farthest <= r.maxRadius + 0.16001 && c.revealed > 0 && c.count > 0 && c.drawn > 0), JSON.stringify(r.entered.caves.map((c) => ({ id: c.id, minimum: c.minimum, farthest: c.farthest, updates: c.updates, count: c.count, revealed: c.revealed }))));
...
const hubPerspective = () => withPage("hub perspective", hubPage(src), async (b) => {
  ...
  await b.send("Input.dispatchKeyEvent", { type: "keyDown", key: "w", text: "w" });
  const caveEntered = await b.evaluate(`new Promise((resolve) => { const B = window.__ooga, start = performance.now(), tick = () => { if (B.cameraCave.playerIndex === 4 && B.cameraCave.index === 4 || performance.now() - start > 6000) { const cave = B.crew.player, p = cave.root.position, h = cave.root.rotation.y, m = B.mouths.find((v) => v.id === "c1"), eye = [p.x + Math.sin(h) * 0.16, p.y - cave.baseY + cave.headOffset * 0.95, p.z + Math.cos(h) * 0.16]; resolve({ scene: B.scene, playerIndex: B.cameraCave.playerIndex, cameraIndex: B.cameraCave.index, actor: [p.x, p.y, p.z], apron: [m.apron.x, m.apron.z], mouth: [m.x, m.z], eyeError: Math.hypot(eye[0] - B.camera.position.x, eye[1] - B.camera.position.y, eye[2] - B.camera.position.z) }); } else requestAnimationFrame(tick); }; tick(); })`);
  await b.send("Input.dispatchKeyEvent", { type: "keyUp", key: "w" });
  await b.send("Input.dispatchKeyEvent", { type: "keyDown", key: "s", text: "s" });
  ...
  await b.send("Input.dispatchKeyEvent", { type: "keyUp", key: "s" });
  record("hub perspective: first-person eye anchoring survives a low Mirror Cave entry and exit", caveEntered.scene === "hub" && caveEntered.playerIndex === 4 && caveEntered.cameraIndex === 4 && caveEntered.eyeError < 0.001 && caveExited.scene === "hub" && caveExited.playerIndex === 0 && caveExited.cameraIndex === 0 && caveExited.eyeError < 0.001, JSON.stringify({ caveEntered, caveExited }));
...
const matrixCharacterActivation = () => withPage("matrix character activation", hubPage(src), async (b) => {
  const result = await b.evaluate(`(() => {
    ...
    const sample = () => ({
      ...
      portal: B.mirror.portal, nodePortal: B.mirrorCave.node.mirrorPortal,
    ...
    step();
    const exited = sample();
    return { maxRadius: W.maxRadius, outside, cameraInside, entered, exited };
  })()`);
  record("matrix activation: a controlled third-person camera cannot trigger the room while the Ooga remains outside", !result.outside.inside && !result.outside.active && result.cameraInside.cameraIndex === 4 && result.cameraInside.actorIndex === 0 && !result.cameraInside.inside && !result.cameraInside.active && result.cameraInside.radius === 0, JSON.stringify(result));
  record("matrix activation: the Ooga's entrance crossing reveals every glyph layer in that same update", result.entered.actorIndex === 4 && result.entered.inside && result.entered.active && result.entered.radius === result.maxRadius && result.entered.direction === 0 && result.entered.portal && result.entered.nodePortal && result.entered.surfaceGlyphs > 0 && result.entered.rainGlyphs > 0, JSON.stringify(result.entered));
  record("matrix activation: the Ooga's exit crossing closes the mirror and starts retreat even while its camera remains inside", result.exited.actorIndex === 0 && result.exited.cameraIndex === 4 && !result.exited.inside && result.exited.active && result.exited.radius < result.maxRadius && result.exited.direction === -1 && !result.exited.portal && !result.exited.nodePortal, JSON.stringify(result.exited));
});
...
const matrixFirstPersonBoundary = (backend) => withPage(`matrix first-person boundary ${backend}`, hubPage(src, backend === "canvas2d" ? "canvas2d=1" : ""), async (b) => {
  const result = await b.evaluate(`(() => {
    ...
    return { backend: B.renderer.kind, maxRadius: W.maxRadius, views, edgeViews, farther, boundaryBeforeRender, boundary, crossedBeforeRender, crossed, exitedBeforeRender, exited, returned };
  })()`);
  ...
  record(`${label}: actual Ooga movement through the plane activates every glyph layer before the mirror opens`, result.crossedBeforeRender.player === 4 && result.crossedBeforeRender.camera === 4 && result.crossedBeforeRender.actorLocal[2] < 0.5 && result.crossedBeforeRender.inside && result.crossedBeforeRender.active && result.crossedBeforeRender.radius === result.maxRadius && result.crossedBeforeRender.direction === 0 && result.crossedBeforeRender.nodePortal && result.crossedBeforeRender.surfaceGlyphs > 0 && result.crossedBeforeRender.rainGlyphs > 0 && result.crossed.portal && !result.crossed.surfaceDrawn, JSON.stringify({ beforeRender: result.crossedBeforeRender, rendered: result.crossed }));
  record(`${label}: the eye's outward crossing closes the fixed mirror before rendering while glyph retraction proceeds independently`, result.exitedBeforeRender.player === 0 && result.exitedBeforeRender.camera === 0 && result.exitedBeforeRender.cameraLocal[2] > 0.5 && !result.exitedBeforeRender.inside && result.exitedBeforeRender.active && result.exitedBeforeRender.radius < result.maxRadius && result.exitedBeforeRender.direction === -1 && !result.exitedBeforeRender.nodePortal && !result.exited.portal && !result.returned.portal && result.returned.surfaceDrawn, JSON.stringify({ beforeRender: result.exitedBeforeRender, rendered: result.exited, viewed: result.returned }));
});
