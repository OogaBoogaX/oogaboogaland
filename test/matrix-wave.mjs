...
export const matrixWaveProbe = (prime = () => {}) => {
  ...
  C.viewInside(false); step(true);
  const entered = snapshot(), expanding = measure("expanding"), early = snapshot();
  advance(12); const beforeExit = snapshot();
  ...
  const staged = [];
  for (const radius of [20, 24, 27, 30, 34, 38]) { advance(radius); step(true); staged.push(snapshot()); }
  const full = measure("full"), fullBuffers = C.caves.flatMap((c) => c.nodes.map((n) => n.instanceData));
  ...
  return { backend: R.kind, quality: R.quality, speed: W.speed, retreatSpeed: W.retreatSpeed, frontWidth: W.frontWidth, maxRadius: W.maxRadius, descriptors: Array.from(W.caves), start, entered, early, beforeExit, exit, reverse, reentry, resumed, paths, staged, cavesRestored, restored,
    ...
    measurements: [inactive, expanding, full, retracting, idle], buffersStable: buffers.every((buffer, i) => buffer === fullBuffers[i] && buffer === current[i]), bytes: buffers.reduce((sum, b) => sum + b.byteLength, 0), bufferCount: buffers.length };
};
