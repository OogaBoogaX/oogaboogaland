...
No analytics. The hub weather subscribes to public chain data through the mempool.space websocket. DSB Land contacts the public feed, radio and media services described below. These services receive normal network request information. Zuzu currently uses local mock replies and sends no conversations to an AI provider. The roster lists public contributor handles only. The donation handle and message a visitor types are stored in their own localStorage and nowhere else.
